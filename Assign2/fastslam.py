    ...
    def correction_step(self, sensor_measurements):
        ...
        for measurement in sensor_measurements:
            ...
            if not landmark.observed:
                ...
                #       - use the measurement model to compute the expected measurement
                #       - use the expected measurement and the current particle pose to compute the landmark position
                #       - initialize the EKF of the landmark with the landmark position and a large covariance
                landmark.mu = robot_pose[:2] + np.vstack([measurement.z_range * cos(measurement.z_bearing+robot_pose[2]), measurement.z_range * sin(measurement.z_bearing + robot_pose[2])])
                ...
                normalized_angle = normalize_angle(measurement.z_bearing - expected_z[1])
                z = np.vstack([measurement.z_range - expected_z[0], normalized_angle])
                
                # TODO: update the mean and covariance of the EKF for this landmark
                landmark.mu = landmark.mu + np.dot(K, z)
                ...
                A = 2 * np.pi * np.sqrt(np.linalg.det(Q))
                B = np.dot(z.T, np.linalg.inv(Q))
                self.weight *= 1/ A * np.exp(-0.5 * np.dot(B, z))
